...
const result = recommend({ currentFunctionId: "ii", recentFunctionIds: ["vi", "ii"], visibleFunctionIds: ["I", "ii", "IV", "V", "vi"] });
check(result.bestMatch?.functionId === "V", "ii should recommend V");
const route = manhattanRoute({ column: 0, row: 0 }, { column: 2, row: 2 });
check(route.every((p, i) => i === 0 || p.column === route[i - 1]!.column || p.row === route[i - 1]!.row), "route must be orthogonal");
