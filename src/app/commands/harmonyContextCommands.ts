...
export interface SetTonicPayload { readonly tonic: number; readonly nowIso: string; }
export type SetTonicCommand = ProjectCommand<SetTonicPayload> & { readonly type: "harmony/set-tonic" };

export function setTonic(project: Project, command: SetTonicCommand): AppliedCommand {
  const previous = project.tonic;
  return {
    project: Object.freeze({ ...project, tonic: ((command.payload.tonic % 12) + 12) % 12, updatedAt: command.payload.nowIso }),
    inverse: { type: "harmony/set-tonic", payload: { tonic: previous, nowIso: command.payload.nowIso } },
  };
}
...
export type SwitchModuleCommand = ProjectCommand<SwitchModulePayload> & { readonly type: "harmony/switch-module" };
...
export function switchModule(project: Project, command: SwitchModuleCommand): AppliedCommand {
  ...
  if (previousModule === command.payload.destinationModule) {
    return {
      project,
      inverse: { type: "harmony/switch-module", payload: { destinationModule: previousModule, resolutions: {}, nowIso: command.payload.nowIso } },
    };
  }
  ...
  const steps: ProgressionStep[] = project.progression.steps.map((step) => {
    ...
    if (!chosen) throw new Error(`Ambiguous module switch for step ${step.id} requires an explicit resolution`);
  ...
  return {
    ...
    inverse: {
      ...
      payload: { destinationModule: previousModule, resolutions: Object.freeze(previousIdentities), nowIso: command.payload.nowIso },
    },
  };
}
