...
export function startTemporaryBranch(
  progression: Progression,
  id: string,
  originStepId?: string,
  compositionIntent: CompositionIntent = "neutral",
): TemporaryBranch {
  if (originStepId !== undefined && indexOfStep(progression, originStepId) < 0)
    throw new RangeError(`Unknown branch origin step: ${originStepId}`);
...
export function appendBranchStep(branch: TemporaryBranch, step: ProgressionStep): TemporaryBranch {
  if (branch.steps.some((candidate) => candidate.id === step.id))
    throw new Error(`Duplicate branch step id: ${step.id}`);
  return Object.freeze({ ...branch, steps: Object.freeze([...branch.steps, step]) });
}

export function setBranchRejoin(
  progression: Progression,
  ...
  if (branch.originAtEnd) {
    if (rejoinStepId !== undefined)
      throw new Error(
        "A branch starting at the progression end cannot rejoin an earlier original step",
      );
  ...
  if (rejoinIndex <= originIndex)
    throw new RangeError("Rejoin step must occur after the branch origin");
...
export function compareBranch(progression: Progression, branch: TemporaryBranch): BranchComparison {
  if (branch.originAtEnd) {
    return Object.freeze({
      prefix: progression.steps,
      originalInterval: Object.freeze([]),
  ...
  const rejoinIndex = branch.rejoinStepId
    ? indexOfStep(progression, branch.rejoinStepId)
    : progression.steps.length;
  if (branch.rejoinStepId && rejoinIndex <= originIndex)
    throw new RangeError("Rejoin step must occur after the branch origin");
  return Object.freeze({
...
export function branchRecommendationPath(
  progression: Progression,
  branch: TemporaryBranch,
...
export function commitBranch(
  progression: Progression,
  branch: TemporaryBranch,
  selectedBranchStepIds?: readonly string[],
): Progression {
  if (!branch.originAtEnd && !branch.rejoinStepId)
    throw new Error("Mid-progression branch requires an explicit rejoin point before commit");
  const comparison = compareBranch(progression, branch);
  const selected = selectedBranchStepIds
    ? branch.steps.filter((step) => selectedBranchStepIds.includes(step.id))
    : branch.steps;
  if (selectedBranchStepIds && selected.length === 0)
    throw new Error("Selective branch commit requires at least one selected branch step");
