...
export function HarmonyDetails({ chord }: { readonly chord: ChordDefinition | null }) {
  ...
  const naturalVariant = fn.moduleId === "dark-harmony" && fn.functionId === "V"
    ? "Natural-minor variant: v"
    : fn.moduleId === "dark-harmony" && fn.functionId === "vii°"
      ? "Natural-minor variant: VII"
      : null;
  const concept = fn.category === "neapolitan"
  ...
  return (
    <section className="harmony-details" aria-label="Harmony details">
      <h3>Harmony</h3>
      <dl>
        <div><dt>Function</dt><dd>{fn.functionId}</dd></div>
        <div><dt>Chord</dt><dd>{chord.spelling.symbol}</dd></div>
        <div><dt>Category</dt><dd>{fn.category.replaceAll("-", " ")}</dd></div>
