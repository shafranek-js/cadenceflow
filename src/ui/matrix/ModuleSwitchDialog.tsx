...
export function ModuleSwitchDialog({
  ...
  const confirm = () => {
    ...
    for (const item of ambiguous) {
      ...
      resolved[item.stepId] =
        selected === "keep-original"
          ? "keep-original"
          : (item.alternatives.find((candidate) => candidate.functionId === selected) ??
            "keep-original");
    }
  ...
  return (
    <div className="dialog-backdrop" role="presentation">
      ...
      >
        ...
        <p>
          Unambiguous functions will convert automatically. Choose how to handle the remaining
          steps.
        </p>
            ...
            <label key={item.stepId}>
              ...
              <select
                ...
                onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                  setSelections(Object.freeze({ ...selections, [item.stepId]: event.target.value }))
                }
              >
                <option value="keep-original">Keep Original</option>
                {item.alternatives.map((candidate) => (
                  <option key={candidate.functionId} value={candidate.functionId}>
