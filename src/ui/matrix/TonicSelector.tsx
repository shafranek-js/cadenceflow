...
export function TonicSelector({ tonic, mode, onChange }: {
    ...
    <aside className="tonic-selector" aria-label="Set The Key">
      ...
      <div className="tonic-buttons">
        {PITCH_CLASSES.map((pc) => {
          ...
          return <button key={pc} type="button" className={pc === tonic ? "is-active" : ""} aria-pressed={pc === tonic} aria-label={`Set key ${label}`} onClick={() => onChange(pc)}>{label}</button>;
        })}
      </div>
    </aside>
  );
