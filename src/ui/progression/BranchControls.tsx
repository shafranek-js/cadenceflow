  ...
  if (!branch) {
    return (
      <div className="branch-controls">
        <select
          aria-label="Branch origin"
          value={origin}
          onChange={(event: ChangeEvent<HTMLSelectElement>) => setOrigin(event.target.value)}
        >
          <option value="end">At progression end</option>
          {project.progression.steps.map((step, index) => (
            <option key={step.id} value={step.id}>
              After {stepLabel(step, index)}
            </option>
          ))}
        </select>
        <button type="button" onClick={() => onStart(origin === "end" ? undefined : origin)}>
          Explore Alternative
        </button>
      </div>
    );
  }
  const originIndex = branch.originAtEnd
    ? project.progression.steps.length - 1
    ...
    <div className="branch-controls active" data-testid="branch-controls-active">
      ...
      {!branch.originAtEnd ? (
        <select
          ...
          value={branch.rejoinStepId ?? ""}
          onChange={(event: ChangeEvent<HTMLSelectElement>) =>
            onRejoin(event.target.value || undefined)
          }
        >
          <option value="">Choose rejoin…</option>
          {rejoinOptions.map((step, offset) => (
            <option key={step.id} value={step.id}>
              {stepLabel(step, originIndex + 1 + offset)}
            </option>
          ))}
        </select>
      ) : (
        <span>Branch from progression end</span>
      )}
      <button
        type="button"
        disabled={branch.steps.length === 0 || (!branch.originAtEnd && !branch.rejoinStepId)}
        onClick={onCommitWhole}
      >
        Commit Branch
      </button>
      <button
        type="button"
        disabled={
          selectedBranchStepIds.length === 0 || (!branch.originAtEnd && !branch.rejoinStepId)
        }
