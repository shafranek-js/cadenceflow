...
test("US4 switches unambiguous progression from Major to Tonal Minor without losing steps", async ({ page }) => {
  await page.goto("/");
...
test("US4 never guesses an ambiguous module conversion silently", async ({ page }) => {
  await page.goto("/");
  await page.getByTestId("chord-card-bVII").getByRole("button", { name: /Add bVII to progression/i }).click();
  await page.getByRole("button", { name: /Dark Harmony/i }).click();
