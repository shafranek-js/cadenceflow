...
import { mapFunctionAcrossModules, planModuleSwitch } from "../../../src/domain/harmony/moduleSwitch";
...
describe("module conversion", () => {
  it("maps unambiguous core functions", () => {
    const result = mapFunctionAcrossModules({ moduleId: "progressions", functionId: "I", category: "core" }, "dark-harmony");
    expect(result?.functionId).toBe("i");
  });
  it("does not guess borrowed functions", () => {
    const result = mapFunctionAcrossModules({ moduleId: "progressions", functionId: "bVII", category: "modal-interchange" }, "dark-harmony");
    expect(result).toBeNull();
  });
  it("offers multiple explicit alternatives plus Keep Original for ambiguous functions", () => {
    const step: ChordStep = {
      id: "s1", kind: "chord", harmonicFunction: { moduleId: "progressions", functionId: "bVII", category: "modal-interchange" },
