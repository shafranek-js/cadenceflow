...
describe("US3 progression-step independence", () => {
  it("snapshots repeated chords independently and preserves explicit template settings after Add", () => {
    ...
    project = patchMatrixTemplate(project, {
      ...
      payload: {
        functionId: "I",
        performanceOverrides: { articulation: "arp-up", masterVelocity: 92 },
        nowIso: T1,
      },
    }).project;
    ...
    populated = editStepPerformance(populated, {
      ...
      payload: {
        stepId: "step-1",
        performance: { masterVelocity: 55, articulation: "humanized" },
        nowIso: T1,
      },
    }).project;
    ...
    expect(
      populated.moduleTemplateStates.progressions.cards.I?.explicitOverrides.performance
        ?.masterVelocity,
    ).toBe(92);
  });
  ...
  it("inherits defaults per parameter without retroactively changing existing steps", () => {
    ...
    const changedDefaults: Project = Object.freeze({
      ...
      defaults: Object.freeze({
        piano: Object.freeze({
          ...
          performance: Object.freeze({
            ...project.defaults.piano.performance,
            masterVelocity: 68,
      ...
      progression: Object.freeze({
        ...project.progression,
        steps: Object.freeze([beforeDefaultsChange]),
      }),
    });
  ...
  it("resets Matrix templates without touching existing My Progression snapshots", () => {
    ...
    const afterCardReset = resetCardTemplate(project, {
      type: "matrix-template/reset-card",
      payload: { functionId: "I", nowIso: T1 },
    }).project;
    ...
    const afterAllReset = resetMatrixScope(afterCardReset, {
      type: "matrix-template/reset-scope",
      payload: { scope: "all-modules", nowIso: T1 },
    }).project;
  ...
  it("reorders the whole immutable step object with all local settings intact", () => {
    ...
    const reordered = reorderStep(project, {
      type: "progression/reorder-step",
      payload: { stepId: "second", targetIndex: 0, nowIso: T1 },
    }).project;
  ...
  it("Replace Step changes harmonic content only and Reset Step Performance preserves variant and duration", () => {
    ...
    const variant = Object.freeze({
      ...
      extensions: Object.freeze([9 as const]),
    });
    ...
    project = patchMatrixTemplate(project, {
      ...
      payload: {
        ...
        harmonicVariantOverride: variant,
        performanceOverrides: { masterVelocity: 111, articulation: "humanized" },
        nowIso: T1,
      },
    }).project;
    ...
    const replacedProject = replaceStep(project, {
    ...
    const resetProject = resetStepPerformance(project, {
    ...
    expect(reset.performance.masterVelocity).toBe(
