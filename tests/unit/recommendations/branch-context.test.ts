...
describe("branch recommendation context", () => {
  it("each branch step becomes context for the next ranking", () => {
    ...
    let path = branchRecommendationPath(p, branch)
      .filter((x) => x.kind === "chord")
      .map((x) => x.harmonicFunction.functionId);
    let result = recommend({
      moduleId: "progressions",
      currentFunctionId: path.at(-1)!,
      recentFunctionIds: path,
      visibleFunctionIds: visible,
    });
    expect(result.bestMatch?.functionId).toBe("IV");

    branch = appendBranchStep(branch, step("b2", "IV"));
    path = branchRecommendationPath(p, branch)
      .filter((x) => x.kind === "chord")
      .map((x) => x.harmonicFunction.functionId);
    result = recommend({
      moduleId: "progressions",
  ...
  it("Composition Intent changes ranking without removing valid candidates", () => {
    ...
    const tension = recommend({
      moduleId: "progressions",
      currentFunctionId: "I",
      recentFunctionIds: ["I"],
      visibleFunctionIds: visible,
      compositionIntent: "build-tension",
    });
    const neutralV =
      [neutral.bestMatch, ...neutral.alternatives].find((x) => x?.functionId === "V")?.score ?? 0;
    const tensionV =
      [tension.bestMatch, ...tension.alternatives].find((x) => x?.functionId === "V")?.score ?? 0;
